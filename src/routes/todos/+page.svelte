<script lang="ts">
  ...
  async function add() {
    ...
    await pb
      .collection("todos")
      .create({ text: v, user: pb.authStore.record.id, done: false });
    text = "";
  }
...
<section class="space-y-6">
  <h1 class="text-3xl font-bold text-emerald-600 mb-4">Your Todos</h1>
  <div class="p-4 bg-red-500 text-white">TAILWIND TEST</div>
  ...
  >
    <input
      class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 outline-none ring-0 focus:border-gray-400"
      bind:value={text}
      placeholder="What needs doing?"
    />
    <button
      type="submit"
      class="rounded-xl border px-4 py-2 text-sm font-medium disabled:opacity-50 hover:bg-gray-50 active:scale-[.99] transition"
      disabled={!text.trim()}
    >
      Add
    </button>
      ...
      <ul class="divide-y">
        {#each items as t (t.id)}
          <li class="flex items-center gap-3 py-3">
            ...
            />
            <span
              class={t.done ? "line-through text-gray-400" : "text-gray-900"}
              >{t.text}</span
            >
            <button
